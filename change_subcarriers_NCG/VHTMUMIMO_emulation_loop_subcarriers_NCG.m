...
name_save_folder = strcat('../emulation_results/020822/change_subcarriers_NCG/no_subc_', num2str(numSTSelected));
...
[functpowerST, gradPower] = power_constraint_gradient_symbolic(numSTSVec, numTx, v1_i, v2_i);
[functobjectiveST, gradObjective] = objective_function_gradient_symbolic(numSTSVec, numTx, v1_i, v2_i);
    ...
    for num_iter=1:end_iter
        ...
        for uIdx = 1:numUsers
            ...
            for s_i = 1:numST
                ...
                [Vtilde_rec, Vtildecompl] = beamformingFeedbackQuantization_complete(V_subcarr, numTx, numSTSVec(uIdx));
                if length(size(V_mat)) == 2
        ...
        %% Attack vector - NCG to enforce constraints
        ...
        while ~ done
            try
                [mat_rec_attacker, num_iterations] = applyNCG_smart(mat_rec_attacker_init, ...
                    size_x, mat_rec{2}, numST, numSTSVec, numTx, numSTSelected, mat_rec{1}, ...
